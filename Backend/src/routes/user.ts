...
userRouter.post("/signup", async (req: Request, res: Response):Promise<any> => {
  ...
  const userD = await user2.create({
    username,
    email,
  ...
  if (userD) {
    const token = jwt.sign({ userId: userD._id }, process.env.JWT_SECRET as string);
...
userRouter.post("/signin", async (req:Request,res:Response):Promise<any>=>{
 ...
 const token=jwt.sign({userId:p._id},process.env.JWT_SECRET as string);
